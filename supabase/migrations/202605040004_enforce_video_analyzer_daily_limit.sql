...
begin
  ...
  do update set
    ...
    updated_at = now()
  where daily_usage.analysis_count < v_daily_analysis_limit
  returning
    daily_usage.usage_date,
    daily_usage.analysis_count
  into
    v_recorded_usage_date,
    v_recorded_analysis_count;

  if not found then
    raise exception 'Daily video analysis limit reached. Try again tomorrow.'
      using errcode = 'P0001';
  end if;

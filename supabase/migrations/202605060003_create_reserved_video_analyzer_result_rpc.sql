create or replace function public.record_reserved_video_analyzer_result(
  p_result jsonb,
  p_clip jsonb,
  p_context jsonb
)
...
begin
  if v_current_user_id is null then
    raise exception 'record_reserved_video_analyzer_result requires an authenticated user'
      using errcode = '28000';
  end if;
  ...
  if coalesce(v_current_daily_count, 0) <= 0 then
    raise exception 'Reserved video analysis usage is required before saving result.'
      using errcode = 'P0001';
  end if;
  ...
  on conflict (user_id, app_analysis_id)
  do update set
...
revoke all on function public.record_reserved_video_analyzer_result(jsonb, jsonb, jsonb) from public;

grant execute on function public.record_reserved_video_analyzer_result(jsonb, jsonb, jsonb) to authenticated;
